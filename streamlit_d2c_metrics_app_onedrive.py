"""
SureBright D2C Metrics & Attribution - Streamlit App
...
def main() -> None:
    ...
    with st.sidebar:
        ...
        source_mode = st.radio(
            "Source",
            ["Upload file", "OneDrive / SharePoint link"],
            horizontal=False,
        )
        ...
        if source_mode == "Upload file":
            ...
            onedrive_url = st.text_input(
                "OneDrive / SharePoint file link",
                placeholder="Paste the sharing link to the Excel/CSV file",
                help="Best: create an 'Anyone with the link can view' file link. For private business links, provide a Graph token below.",
            )
            default_token = ""
            try:
                default_token = st.secrets.get("MS_GRAPH_TOKEN", "")
            except Exception:
                default_token = ""
            graph_token = st.text_input(
                "Optional Microsoft Graph access token",
                value=default_token,
                type="password",
                help="Only needed if your OneDrive/SharePoint file is private and the anonymous link cannot download.",
            )
    ...
    if source_mode == "Upload file" and uploaded_file is None:
        ...
        st.stop()
    if source_mode == "OneDrive / SharePoint link" and not onedrive_url.strip():
        st.info("Paste a OneDrive or SharePoint file link to generate metrics.")
        st.stop()
    ...
    try:
        if source_mode == "Upload file":
            ...
            with st.spinner("Downloading source file from OneDrive..."):
                file_bytes, source_filename, download_url_used = fetch_onedrive_file(
                    onedrive_url,
                    access_token=graph_token or None,
                )
            source_note = f"OneDrive link via {download_url_used}"
        ...
        st.error("Could not process the source. Check that it is a raw D2C export with JSON payloads and that the link is downloadable.")
        st.exception(exc)
        st.stop()
